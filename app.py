...
# 設置頁面配置
st.set_page_config(page_title="妍菲天堂 - 全能數據搜尋中心 v4.4", page_icon="🔍", layout="wide")
...
def fetch_api_data(endpoint):
    try:
        ...
        if res.status_code == 200:
            json_data = res.json()
            if isinstance(json_data, dict) and "rows" in json_data: return json_data["rows"]
            return json_data
        return None
    except: return None
...
st.sidebar.markdown("<h2 style='text-align:center; color:white;'>妍菲天堂 v4.4</h2>", unsafe_allow_html=True)
...
if menu == "首頁入口":
    ...
    stats_resp = fetch_api_data("/api/stats")
    if stats_resp and stats_resp.get("ok"):
        s = stats_resp["stats"]
        cols = st.columns(5)
        cols[0].metric("📦 物品", s.get("items", 0))
        cols[1].metric("👹 怪物", s.get("monsters", 0))
        cols[2].metric("💎 掉落", s.get("drops", 0))
        cols[3].metric("🗡️ 武器", s.get("weapons", 0))
        cols[4].metric("🛡️ 防具", s.get("armors", 0))
    ...
    if query:
        with st.spinner("正在穿透資料庫檢索中..."):
            # 平行或依序獲取資料
            weapons = fetch_api_data("/api/weapons")
            armors = fetch_api_data("/api/armors")
            items = fetch_api_data("/api/items")
            monsters = fetch_api_data("/api/monsters")
            drops = fetch_api_data("/api/drops")
            ...
            # --- 顯示其他道具 ---
            if items:
                i_res = [i for i in items if query in str(i.get('name','')) and query not in str([w.get('name') for w in weapons if weapons])]
                if i_res:
...
st.markdown("---")
st.markdown('<div style="text-align:center; color:#999;">© 2026 妍菲天堂 Lineage Portal v4.4 | 整合搜尋引擎版</div>', unsafe_allow_html=True)
