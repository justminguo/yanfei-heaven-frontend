...
# 設置頁面配置
st.set_page_config(page_title="妍菲天堂 - 全能數據搜尋中心 v5.5", page_icon="🔍", layout="wide")
...
# --- 全站視覺強制鎖定 CSS (解決深色模式衝突) ---
st.markdown(f"""
    <style>
    /* 1. 強制全站背景與基礎文字顏色 (無視系統深色模式) */
    ...
    .category-header {{ 
        ...
        font-size: 1.5em; 
        font-weight: bold; 
        border-bottom: 4px solid #004494 !important;
    }}
...
st.sidebar.markdown("<h2 style='text-align:center; color:white;'>妍菲天堂 v5.5</h2>", unsafe_allow_html=True)
...
if menu == "首頁入口":
    ...
    st.markdown("""
        <div class="result-card">
            <h3>🏺 聖物系統</h3>
            <p>透過收集特定的怪物材料，玩家可以強化專屬聖物，獲得永久性的力量、敏捷或魔法加成。</p>
        </div>
        <div class="result-card">
            <h3>🤖 智能自動練功</h3>
            <p>專為上班族玩家設計，提供最穩定的掛機環境。智能判斷回城補給、自動販賣垃圾物品。</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    st.table(pd.DataFrame({
        ...
        "獎勵內容": ["5,000 元寶 + 藥水包", "10,000 元寶 + 變身卷軸", "神之飾品袋", "傳說武器箱(限時)"]
    }))

st.markdown("---")
st.markdown('<div style="text-align:center; color:#999;">© 2026 妍菲天堂 v5.5 | 視覺強化鎖定版</div>', unsafe_allow_html=True)
