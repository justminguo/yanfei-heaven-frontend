...
# 設置頁面配置
st.set_page_config(page_title="妍菲天堂 - 全能數據搜尋中心 v5.1", page_icon="🔍", layout="wide")
...
def load_all_data():
    ...
    if os.path.exists(LOCAL_DATA_FILE):
        ...
        # 如果檔案不見了，手動生成幾筆基礎資料避免搜尋全空
        all_data["weapons"] = [
            {"name": "瑟魯基之劍", "dmg_small": 16, "dmg_large": 10, "hit": 2, "dmg_add": 0, "str": 0, "material": "鐵", "weight": 40, "hands": "單手", "info": "經典武器"},
            {"name": "細劍", "dmg_small": 11, "dmg_large": 6, "hit": 0, "dmg_add": 0, "str": 0, "material": "銀", "weight": 35, "hands": "單手", "info": "不死系剋星"}
        ]
        all_data["monsters"] = [{"name": "黑豹", "level": 70, "hp": 5000, "mp": 200, "ac": -80, "exp": 10000, "info": "BOSS"}]
    return all_data
...
st.sidebar.markdown("<h2 style='text-align:center; color:white;'>妍菲天堂 v5.1</h2>", unsafe_allow_html=True)
...
if menu == "首頁入口":
    ...
    cols = st.columns(5)
    cols[0].metric("📦 物品", len(data_source.get("items", [])) + len(data_source.get("weapons", [])) + len(data_source.get("armors", [])))
    ...
    query = st.text_input("💡 立即輸入關鍵字 (例如：劍、黑豹、金幣...)", placeholder="輸入即自動搜尋...", key="search_input")
...
st.markdown("---")
st.markdown('<div style="text-align:center; color:#999;">© 2026 妍菲天堂 v5.1 | 數據精確搜尋版</div>', unsafe_allow_html=True)
